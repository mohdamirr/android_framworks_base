...
public class NavigationBarEdgePanel extends View implements NavigationEdgeBackPlugin {
    ...
    private void triggerBack(boolean triggerLongSwipe) {
        ...
        boolean shouldVibrate = Math.abs(mVelocityTracker.getXVelocity()) < 500 || SystemClock.uptimeMillis() - mVibrationTime >= GESTURE_DURATION_FOR_CLICK_MS;
        triggerVibrationClick(shouldVibrate);
    ...
    private void handleMoveEvent(MotionEvent event) {
        ...
        if (!mDragSlopPassed && touchTranslation > mSwipeTriggerThreshold) {
            ...
            triggerVibrationTick(mDragSlopPassed);
    ...
    private void triggerVibrationClick(boolean vibrate) {
        if (mVibrator == null || !mEdgeHapticEnabled || !vibrate) {
            return;
        }
        AsyncTask.execute(
    ...
    private void triggerVibrationTick(boolean vibrate) {
        if (mVibrator == null || !mEdgeHapticEnabled || !vibrate) {
            return;
        }
        AsyncTask.execute(
